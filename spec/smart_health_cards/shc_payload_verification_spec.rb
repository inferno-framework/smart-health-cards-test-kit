RSpec.describe SmartHealthCards::SHCPayloadVerification do
  ...
  describe 'health_card_payload_test' do
    ...
    it 'fails if the JWS payload was not compressed with the deflate algorithm' do

      credential_strings = 'garbage-in-garbage-out'
      result = run(test, { file_download_url: url, url: url, credential_strings: credential_strings })
      #binding.pry
    ...
    it 'fails if the JWS payload contains invalid data' do
      #todo
    end
