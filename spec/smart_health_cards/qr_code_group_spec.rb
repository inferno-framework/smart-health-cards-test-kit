RSpec.describe SmartHealthCards::QrCodeGroup do
  ...
  describe 'qr_code_verifiable_credential_test' do
    let(:test) { group.tests.find { |t| t.id.include?('qr_code_verifiable_credential_test')} }
    ...
    it 'passes if valid shc string is provided' do
      result = run(test, { qr_code_content: qr_content, url: url  })
