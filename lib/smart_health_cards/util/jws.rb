...
module SmartHealthCards
  ...
  class JWS
    class << self
      ...
      # Creates a JWS from a String representation, or returns the SmartHealthCards::JWS object
      # that was passed in
      # @param jws [String, SmartHealthCards::JWS] the JWS string, or a JWS object
      # @param public_key [SmartHealthCards::PublicKey] the public key associated with the JWS
      # @param key [SmartHealthCards::PrivateKey] the private key associated with the JWS
      # @return [SmartHealthCards::JWS] A new JWS object, or the JWS object that was passed in
      def from_jws(jws, public_key: nil, key: nil)
        return jws if jws.is_a?(SmartHealthCards::JWS) && public_key.nil? && key.nil?

        unless jws.is_a?(SmartHealthCards::JWS) || jws.is_a?(String)
          raise ArgumentError,
                'Expected either a SmartHealthCards::JWS or String'
        end
    ...
    #
    # @param key [SmartHealthCards::PrivateKey, nil] the private key used for signing issued health cards
    def key=(key)
    ...
    #
    # @param public_key [SmartHealthCards::PublicKey, nil] the private key used for signing issued health cards
