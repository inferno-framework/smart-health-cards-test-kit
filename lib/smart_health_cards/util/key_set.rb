...
module SmartHealthCards
  ...
  class KeySet
    ...
    # @return [SmartHealthCards::KeySet]
    def self.from_jwks(jwks)
      jwks = JSON.parse(jwks)
      keys = jwks['keys'].map { |jwk| SmartHealthCards::Key.from_jwk(jwk) }
      KeySet.new(keys)
    end

    # Create a new KeySet
    #
    # @param keys [SmartHealthCards::Key, Array<SmartHealthCards::Key>, nil] the initial keys
    ...
    #
    # @param new_keys [SmartHealthCards::Key, Array<SmartHealthCards::Key>, SmartHealthCards::KeySet] the initial keys
    ...
    #
    # @param key [SmartHealthCards::Key]
    # @return [Boolean]
    def include?(key)
      !@key_map[key.kid].nil?
    end
